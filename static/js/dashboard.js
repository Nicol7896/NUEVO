...
function setupFileUpload() {
    ...
    uploadForm.addEventListener('submit', async function(e) {
        ...
        if (!file) {
            showUploadStatus('Por favor seleccione un archivo CSV', 'error');
            return;
        }
        
        if (!file.name.endsWith('.csv')) {
            showUploadStatus('El archivo debe ser de tipo CSV', 'error');
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                showUploadStatus('Archivo cargado exitosamente', 'success');
                // Recargar datos del dashboard
                loadDashboardData();
            } else {
                showUploadStatus(result.error || 'Error al cargar el archivo', 'error');
            }
        } catch (error) {
            console.error('Error en la subida:', error);
            showUploadStatus('Error al procesar el archivo', 'error');
        }
    });
    
    // Agregar botón para volver a datos por defecto
    const resetButton = document.createElement('button');
    resetButton.type = 'button';
    resetButton.className = 'btn btn-secondary mt-2';
    resetButton.innerHTML = '<i class="fas fa-undo"></i> Volver a datos por defecto';
    resetButton.onclick = resetToDefaultData;
    
    uploadForm.appendChild(resetButton);
}
...
/**
 * Volver a datos por defecto
 */
async function resetToDefaultData() {
    try {
        showUploadStatus('Restaurando datos por defecto...', 'info');
        ...
        if (result.success) {
            showUploadStatus('Datos restaurados exitosamente', 'success');
            // Recargar datos del dashboard
            loadDashboardData();
        } else {
            showUploadStatus(result.error || 'Error al restaurar los datos', 'error');
        }
    } catch (error) {
