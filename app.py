...
class DataAnalyzer:
    ...
    def load_custom_data(self, file_path):
        """Cargar datos desde un archivo CSV personalizado"""
        try:
            df = pd.read_csv(file_path)
            required_columns = ['Ciudad', 'Categoría del problema', 'Nivel de urgencia', 'Zona rural', 'Acceso a internet']
            ...
            if 'Prioridad' not in df.columns:
                print("Calculando prioridad para los datos personalizados...")
                df['Prioridad'] = self.calculate_priority_for_df(df)
            
            # Si no hay columna de fecha, agregar una por defecto
            if 'Fecha del reporte' not in df.columns:
                df['Fecha del reporte'] = pd.Timestamp.now().strftime('%Y-%m-%d')
            
            self.custom_df = df
            self.using_custom_data = True
            print(f"✅ Datos personalizados cargados: {len(df)} registros")
            return True
            
        except Exception as e:
            print(f"❌ Error cargando datos personalizados: {e}")
            return False
    
    def calculate_priority_for_df(self, df):
        """Calcular prioridad para un DataFrame específico"""
        priority_scores = []
    ...
    def get_active_df(self):
        """Obtener el DataFrame activo (personalizado o por defecto)"""
        return self.custom_df if self.using_custom_data else self.df
    
    def reset_to_default_data(self):
        """Volver a los datos por defecto"""
        self.using_custom_data = False
        self.custom_df = None
        print("✅ Volviendo a los datos por defecto")
        return True
    ...
    def get_dashboard_metrics(self):
        ...
        df = self.get_active_df()
        total_casos = len(df)
        casos_urgentes = len(df[df['Nivel de urgencia'] == 'Urgente'])
        zona_rural = len(df[df['Zona rural'] == 1])
        sin_internet = len(df[df['Acceso a internet'] == 0])
    ...
    def get_category_distribution(self):
        ...
        distribution = df['Categoría del problema'].value_counts()
        return {
    ...
    def get_urgency_distribution(self):
        ...
        distribution = df['Nivel de urgencia'].value_counts()
        return {
            'labels': distribution.index.tolist(),
            'values': distribution.values.tolist()
        }
...
def api_upload_csv():
    """API para cargar archivo CSV personalizado"""
    try:
        ...
        # Cargar los datos
        if analyzer.load_custom_data(temp_path):
            ...
            return jsonify({
                'success': False,
                'error': 'Error al procesar el archivo. Asegúrese de que contiene las columnas requeridas'
            }), 400
...
def api_reset_data():
    """API para volver a los datos por defecto"""
    try:
        if analyzer.reset_to_default_data():
            return jsonify({
                'success': True,
